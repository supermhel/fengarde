...
# Gap-hunt finding (R2-#3): read_text()'s OSError/UnicodeDecodeError used to
# escape load_webhook_configs uncaught -- a single unreadable or non-UTF-8
# config took DOWN config loading (and thus every other webhook, since
# loading -> run() -> dispatcher never started at all).
def test_unreadable_and_non_utf8_configs_skipped_not_crashed():
    import shared.log as shared_log
    ...
    try:
        with tempfile.TemporaryDirectory() as d:
            ...
            (d / "good.yml").write_text(
                "id: good\nurl: https://example.com/hook\nsecret_env: S\n")
            # A path that is actually a DIRECTORY: read_text() raises
            # OSError (IsADirectoryError).
            (d / "is_a_dir.yml").mkdir()
            # A file with invalid UTF-8 bytes: read_text(encoding="utf-8")
            # raises UnicodeDecodeError.
            (d / "not_utf8.yml").write_bytes(b"id: bad\xff\xfe\nurl: x\n")

            configs = webhooks.load_webhook_configs(d)
            check([c.id for c in configs] == ["good"],
                  f"only the readable/valid config may load, got {[c.id for c in configs]}")
            check(len(fake_log.warnings) == 2,
                  f"both unreadable/non-UTF-8 configs must LOG loudly, got {fake_log.warnings}")
            check(any("is_a_dir.yml" in w for w in fake_log.warnings),
                  "the OSError config (directory) must be named in a warning")
            check(any("not_utf8.yml" in w for w in fake_log.warnings),
                  "the UnicodeDecodeError config must be named in a warning")
    finally:
        shared_log.get_logger = real_get_logger_fn
...
def main():
    ...
    test_unreadable_and_non_utf8_configs_skipped_not_crashed()
