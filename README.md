...
| **Rule boundary guardrail** | ✅ Works for stateful rules; ⚠️ stateless rules untested | Satisfiable ≠ fires, and firing ≠ firing *only when it should*. `eval/attack/fire_check.py` proves all 27 MITRE-tagged rules fire on their own fixture, then replays each stateful rule one under its threshold and spread just past its window and requires silence — catching a rule that is too **loose**, which shows up as false-positive volume months later rather than as a dead rule. 12/12 stateful rules hold; the 15 stateless ones have no generatable near-miss and are reported **untested, not passing** |
