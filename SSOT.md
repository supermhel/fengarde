...
| Opt-in live-infra test lane (`make test-live`, v0.4 P2.6) | **Design/claim — lane exists, not yet executed against real infra** | `services/ws3-indexer/storage/test_opensearch_live.py` (idempotent-upsert, real 409 on stale OCC version, permanent-4xx-not-retried) + redis-parametrized `services/shared/test_runner.py`. Both skip cleanly without a reachable backend; this session added the lane but Docker Desktop wasn't running here, so it has only been proven to SKIP correctly, not to pass against a live cluster. |
