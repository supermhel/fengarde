...
fabricated one:
  ...
  * mutation_robustness        = Phase 4 (2026-09-03): the deterministic
                                       Layer A matrix's overall robustness
                                       (fraction of mutations that PASS on
                                       detection-retained AND fidelity-
                                       retained AND fcr-unchanged), read from
                                       eval/adversarial/out/matrix.latest.json;
                                       None + context note when the lane has
...
# ---------------------------------------------------------------------------
# Phase 4: mutation_robustness source -- the DETERMINISTIC Layer A matrix
# (eval/adversarial/layer_a.py). The matrix is a pure function of the seed:
# same seed -> same byte-identical variant set -> same graded matrix, written
# to a GITIGNORED artifact (the same convention eval/twin/report.latest.json
# uses). The scorecard READS it (never re-runs the whole lane -- that would
# multiply the twin's runtime) and reports the overall + per-axis robustness
# with the matrix as the basis. When the lane has not run yet the metric is
# an honest null with context.mutation_robustness_note naming the reason --
...
def _load_mutation_robustness() -> tuple[Optional[float], dict]:
    ...
    regenerated by layer_a.py, never by this module; reading it does not
...
def _incident_membership_grade(alerts_by_step: list[dict], incidents: list[dict],
    ...
    if not by_id or not chain_set:
        return {"incident_count": 0, "incident_membership_ok": False,
                ...
                # Phase-4 seam: a MUTATED chain can legitimately promote
                # nothing (a mutation that destroys detection is a measured
                # failure, never a crash) -- the early return must carry the
                # same key shape _grade_ws8 reads, or the grading path
                # KeyErrors on exactly the interesting case.
                "cross_step_rule_co_location": {},
...
def run(seed: int = 7) -> dict:
    ...
    report = {
        ...
        "metrics": {
            ...
            # Phase 4: mutation_robustness -- the deterministic Layer A
            # matrix's overall robustness (harness-measured; see
            # _load_mutation_robustness). None + context.mutation_robustness
            # note when the lane hasn't run yet -- never a fabricated number.
        ...
        "context": {
            ...
            # Phase 4: the Layer A mutation-robustness context (real matrix
            ...
            "mutation_robustness_note": (
                ...
                "graded on detection retained + chain fidelity retained + "
                "false-correlation unchanged; a row whose alert survives but "
                "whose causal join breaks is recorded causal_join_broken and "
                "counts AGAINST robustness, never folded into a pass. "
                "Missing here = the lane has not run in this environment yet "
                "(see the matrix source in context)."
                if mutation_matrix else
                "the Layer A matrix artifact (eval/adversarial/out/"
                "matrix.latest.json) was not present when this scorecard ran "
                "-- mutation_robustness stays an honest null; run "
                "eval/adversarial/layer_a.py --seed 7 first (the gate order "
                "ensures it runs before the scorecard)"),
        },
        "finding": (
