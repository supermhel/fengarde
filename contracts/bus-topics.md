...
| `entity.updates`   | WS-9 Resolver, WS-6 Inventory (2026-08-28, new — see ADR-009) | WS-9 (self), WS-3 | `{entity_id, entity_type, tenant_id, entity_value, first_seen_ms, last_seen_ms, attributes}` — deterministic `entity_id` = `sha256(tenant|type|canonical_value)`, idempotent under redelivery | `entity_id` |
| `incident.graph`   | WS-8 Correlation, WS-9 Resolver (2026-08-28, new — see ADR-009) | WS-9, WS-3       | `{version: 1, incident_id, tenant_id, nodes: [type:value…], edges: [{from, to, kind, event_id, ts_ms}], tactic_sources}` — no transitive inference; edges carry `event_id`+`ts_ms` provenance; Phase 3 upgrades to a typed causal DAG via `version: 2` | `incident_id` |
