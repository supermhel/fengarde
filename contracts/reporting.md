...
Frozen cross-repo seam between this repo (`argiem`, open) and `argiem-sec` (private,
paid). Open half: the hook + a generic template backend, both here, both usable
with zero paid dependency. Paid half: `argiem-sec` implements the same backend
...
`argiem-sec`'s asset. This contract is the additive-field boundary between them
— the open pipeline must be complete and useful without `argiem-sec` ever
running.
...
the triage API (`ARGIEM_API_KEY`, see `SECURITY.md` §2).
...
| `http` | POST to `ARGIEM_SEC_REPORT_URL`, response validated against this schema | `argiem-sec` |
...
fail-open pattern — `argiem-sec` is an external HTTP callee, not a new
...
1. **`status` is `"draft"` and only `"draft"` in v0.4.** The enum widens only
   when `argiem-sec`'s legal sign-off gate passes (see its `docs/STATUS.md`).
...
3. **`citations` is optional and may be `[]`.** The open pipeline never reads
   ...
   (`argiem-sec`'s Track C3). A template-backend report has no citations and
...
`argiem-sec`'s legal sign-off gate) says otherwise.
