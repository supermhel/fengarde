...
FENGARDE's storage layer is [OpenSearch](https://opensearch.org/) — the
...
## What FENGARDE does and doesn't turn on
...
(`FENGARDE_API_KEY`) protects the two HTTP write surfaces (triage and inventory
...
doctor script). FENGARDE doesn't pretend that complexity vanishes; it just
